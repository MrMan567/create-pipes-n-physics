...
public final class MomentumField {
    ...
    private record Frame(double ax, double ay, double az,
                         double ox, double oy, double oz, long tick) {}
     ...
     * derivative of the velocity over the step, keyed by the sub-level's unique id.
     */
    public static void record(String subLevelId, double vx, double vy, double vz,
                              double ox, double oy, double oz, double timeStep, long tick) {
        Velocity prev = LAST_VELOCITY.put(subLevelId, new Velocity(vx, vy, vz));
        if (prev == null || timeStep <= 1e-6) return;
        double ax = (vx - prev.vx()) / timeStep;
        double ay = (vy - prev.vy()) / timeStep;
        double az = (vz - prev.vz()) / timeStep;
        Frame last = FRAMES.get(subLevelId);
        if (last != null) {
            ax = last.ax() + (ax - last.ax()) * SMOOTHING;
            ay = last.ay() + (ay - last.ay()) * SMOOTHING;
            az = last.az() + (az - last.az()) * SMOOTHING;
        }
        FRAMES.put(subLevelId, new Frame(ax, ay, az, ox, oy, oz, tick));
    }
    ...
    public static double headOffset(Level level, BlockPos pos) {
        ...
        double accel = Math.sqrt(sq(frame.ax()) + sq(frame.ay()) + sq(frame.az()));
        ...
        double potential = frame.ax() * (world.x - frame.ox())
                + frame.ay() * (world.y - frame.oy())
                + frame.az() * (world.z - frame.oz());
        return Math.clamp(
    ...
    public static void sweep(long now) {
        FRAMES.entrySet().removeIf(e -> now - e.getValue().tick() > STALE_TICKS);
