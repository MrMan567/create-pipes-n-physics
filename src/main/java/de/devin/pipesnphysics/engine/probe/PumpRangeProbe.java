...
public final class PumpRangeProbe {
    ...
    public static PumpRangePayload probe(ServerLevel level, BlockPos pumpPos) {
        ...
        for (Edge edge : graph.edgesOf(pump.index())) {
            ...
            List<PathPoint> seedPath = new ArrayList<>();
            seedPath.add(new PathPoint(pumpPos, true));
            walker.walk(edge, pump.index(), seedPath, pull);
        }
        return new PumpRangePayload(pumpPos, walker.paths);
    }

    /** One visited cell of a range path with its reachability verdict. */
    private record PathPoint(BlockPos pos, boolean reachable) {}
    ...
    private static final class Walker {
        ...
        void walk(Edge edge, int fromNode, List<PathPoint> path, boolean pull) {
            ...
            for (BlockPos cell : ordered) {
                ...
                path.add(new PathPoint(cell, reachable(y, pull)));
            ...
            path.add(new PathPoint(far.pos(), reachable(farY, pull)));
        ...
        void emit(List<PathPoint> path, boolean pull) {
            if (path.size() < 2) return;
            List<Long> packed = new ArrayList<>(path.size());
            List<Boolean> reachable = new ArrayList<>(path.size());
            for (PathPoint point : path) {
                packed.add(point.pos().asLong());
                reachable.add(point.reachable());
            }
            paths.add(new PumpRangePayload.RangePath(packed, reachable, pull));
        }
    }
}
