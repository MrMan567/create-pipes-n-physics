 ...
 *
 * Runs per network per tick after the hydraulic solve, on the server thread. Placement is simulated
...
public final class CentrifugeProcessor {
    ...
    /**
     * Run one un-mix sweep over a network. Gated on the centrifuge config toggles, a registered
     * recipe existing, and the whole contraption spinning fast enough (both the orbital-speed and
     * angular-speed minimums). When it runs, every reactor tank holding a recipe's input fluid is
     * drained and the products are pushed into the network's tanks ranked by density against
     * radius — densest to the outermost (fastest-orbiting) tap.
     */
    ...
    private static void unmix(ServerLevel level, Graph graph, Node reactor, IFluidHandler reactorCap,
        ...
        sinks.sort(Comparator.comparingDouble((Sink sink) -> sink.speed).reversed());
    ...
    private static int maxFittingOps(List<Sink> sinks, List<FluidStack> outputs, int maxOps) {
        List<TankSpace> tanks = new ArrayList<>();
        for (Sink sink : sinks) tanks.add(new TankSpace(heldFluid(sink.handler), freeSpace(sink.handler)));
        // Monotonic in ops (more product needs more room), so the largest that fits is first-fit downward.
        for (int ops = maxOps; ops >= 1; ops--) {
            if (fits(copyOf(tanks), outputs, ops)) return ops;
        }
        return 0;
    }

    /** Simulate placing every output (densest first) into the tanks (outermost first) with claiming. */
    private static boolean fits(List<TankSpace> tanks, List<FluidStack> outputs, int ops) {
        for (FluidStack output : outputs) {
            int need = ops * output.getAmount();
            for (TankSpace tank : tanks) {
                if (need <= 0) break;
                if (!tank.accepts(output)) continue;
                int take = Math.min(need, tank.free);
                if (tank.held.isEmpty()) tank.held = output; // claim this empty tank for this fluid
                tank.free -= take;
                need -= take;
            }
    ...
    private static List<TankSpace> copyOf(List<TankSpace> tanks) {
        List<TankSpace> copies = new ArrayList<>(tanks.size());
        for (TankSpace tank : tanks) copies.add(new TankSpace(tank.held, tank.free));
        return copies;
    }
    ...
    /** One tank's simulated fitting state: the fluid it holds (or has claimed) and its remaining room. */
    private static final class TankSpace {
        private FluidStack held;
        private int free;

        private TankSpace(FluidStack held, int free) {
            this.held = held;
            this.free = free;
        }

        private boolean accepts(FluidStack output) {
            return free > 0 && (held.isEmpty() || FluidStack.isSameFluidSameComponents(held, output));
        }
    }
}
